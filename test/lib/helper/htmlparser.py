...
def getParamsFromHtml(html):
    parse = MyHTMLParser()
    parse.feed(html)
    tokens = parse.getTokenizer()#把html按结构树转换成字典
    result = set()
    for token in tokens:
        
        tagname = token["tagname"].lower() #取出input标志的元素
        if tagname == "input":
            for attibute in token["attibutes"]:
                key, value = attibute
                if key == "name":
                    result.add(value)#参数名放入数组
                    break
        elif tagname == "script": #解析js
            content = token["content"]
            try:
                nodes = pyjsparser.parse(content).get("body", [])#js解析成树状结构
