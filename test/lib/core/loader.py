...
def load_file_to_module(file_path):#执行插件
    if '' not in importlib.machinery.SOURCE_SUFFIXES:
        importlib.machinery.SOURCE_SUFFIXES.append('')
    try:
        module_name = 'plugin_{0}'.format(get_filename(file_path, with_ext=False))
        spec = importlib.util.spec_from_file_location(module_name, file_path, loader=PocLoader(module_name, file_path))#加载插件
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod) #执行插件，后面就可以调用插件mod的方法
        return mod
